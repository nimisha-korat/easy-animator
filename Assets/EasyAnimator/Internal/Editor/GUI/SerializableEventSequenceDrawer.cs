...
using Sequence = EasyAnimator.EasyAnimatorEvent.Sequence.Serializable;
...
namespace EasyAnimator.Editor
{
    ...
    public sealed class SerializableEventSequenceDrawer : PropertyDrawer
    {
        ...
        public static UnityAction Repaint = EasyAnimatorGUI.RepaintEverything;
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            using (var context = Context.Get(property))
            {
                ...
                if (events != null)
                    height += EventSequenceDrawer.Get(events).CalculateHeight(events) + EasyAnimatorGUI.StandardSpacing;
        ...
        private float CalculateEventHeight(Context context, int index)
        {
            ...
            var height = index < context.Times.Count - 1 ?
                EasyAnimatorGUI.LineHeight + EasyAnimatorGUI.StandardSpacing :
                0;// End Events don't have a Name.

            // Time.
            height += EventTimeAttribute.GetPropertyHeight(null, null) + EasyAnimatorGUI.StandardSpacing;
            ...
            height += EasyAnimatorGUI.StandardSpacing;
            ...
            using (var context = Context.Get(property))
            {
                ...
                if (sequence != null)
                {
                    using (ObjectPool.Disposable.AcquireContent(out label, "Runtime Events",
                        $"The runtime {nameof(EasyAnimatorEvent)}.{nameof(Sequence)} created from the serialized data above"))
                    {
        ...
        private void DoHeaderGUI(ref Rect area, GUIContent label, Context context)
        {
            ...
            {
                var addEventArea = EasyAnimatorGUI.StealFromRight(ref headerArea, headerArea.height, EasyAnimatorGUI.StandardSpacing);
            ...
            {
                label.text = EasyAnimatorGUI.GetNarrowText(label.text);
                ...
                isEndEvent ? Strings.Tooltips.EndTime : Strings.Tooltips.CallbackTime))
            {
                ...
                if (index < context.Times.Count)
                {
                    ...
                    else// Event time property was correctly retrieved.
                    {
                        ...
                        if (EasyAnimatorGUI.TryUseClickEvent(timeArea, 2))
                            normalizedTime = float.NaN;
                ...
                {
                    EasyAnimatorUtilities.Assert(index == 0, "Dummy end event index != 0");
                    EditorGUI.BeginChangeCheck();
        ...
        public static void DoCallbackGUI(ref Rect area, Context context, int index, bool autoSort, string callbackLabel)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (index < context.Callbacks.Count)
                {
                    ...
                    if (events != null)
                    {
                        var EasyAnimatorEvent = index < events.Count ? events[index] : events.endEvent;
                        if (EasyAnimatorEvent.IsNullOrDummy(EasyAnimatorEvent.callback))
                        {
            ...
            out string nameLabel, out string timeLabel, out string callbackLabel, out float defaultTime, out bool isEndEvent)
        {
            if (index >= context.Times.Count - 1)
            {
                ...
                defaultTime = EasyAnimatorEvent.Sequence.GetDefaultNormalizedEndTime(
        ...
        private static void WrapEventTime(Context context, ref float normalizedTime)
        {
            ...
            if (transition != null && transition.IsLooping)
            {
                if (normalizedTime == 0)
                    ...
                    normalizedTime = EasyAnimatorEvent.AlmostOne;
                else
                    normalizedTime = EasyAnimatorUtilities.Wrap01(normalizedTime);
            }
        }
        ...
        public static void AddEvent(Context context, float normalizedTime)
        {
            ...
            else
            {
                ...
                if (float.IsNaN(normalizedTime))
                {
                    ...
                    var previousTime = context.Times.Count >= 3 ?
                        ...
                        EasyAnimatorEvent.Sequence.GetDefaultNormalizedStartTime(transition.Speed);

                    var endTime = context.Times.GetElement(context.Times.Count - 1).floatValue;
                    if (float.IsNaN(endTime))
                        endTime = EasyAnimatorEvent.Sequence.GetDefaultNormalizedEndTime(transition.Speed);
            ...
            var events = context.Sequence?.InitializedEvents;
            if (events != null)
                events.Add(normalizedTime, EasyAnimatorEvent.DummyCallback);
        ...
        public static void RemoveEvent(Context context, int index)
        {
            ...
            if (index >= context.Times.Count - 1)
            {
                ...
                if (events != null)
                {
                    events.endEvent = new EasyAnimatorEvent(float.NaN, null);
                }
            }
                ...
                set
                {
                    if (Times != null && value >= 0 && (value < Times.Count || Times.Count == 0))
                    {
                        ...
                        else
                        {
                            ...
                            normalizedTime = EasyAnimatorEvent.Sequence.GetDefaultNormalizedEndTime(speed);
                        }
