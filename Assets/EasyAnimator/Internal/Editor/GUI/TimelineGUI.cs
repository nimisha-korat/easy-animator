...
namespace EasyAnimator.Editor
{
    ...
    public sealed class TimelineGUI : IDisposable
    {
        ...
        public static Texture EventIcon => _EventIcon != null ?
            _EventIcon :
            (_EventIcon = EasyAnimatorGUI.LoadIcon("Animation.EventMarker"));
        ...
        public float SecondsToPixels(float seconds) => EasyAnimatorUtilities.Round((seconds - _MinTime) * _SecondsToPixels);
        ...
        private void DoGUI(SerializableEventSequenceDrawer.Context context, out float addEventNormalizedTime)
        {
            ...
            if (EasyAnimatorUtilities.TryGetWrappedObject(transition, out ITransitionGUI gui))
        ...
        {
            if (speed < 0)
                return endTime > 0 ? 0 : (endTime - EasyAnimatorPlayable.DefaultFadeDuration) * -speed;
            else
                return endTime < duration ? duration : endTime + EasyAnimatorPlayable.DefaultFadeDuration * speed;
        }
        ...
        private void GatherEventTimes(SerializableEventSequenceDrawer.Context context)
        {
            ...
            _EndTime = EasyAnimatorEvent.Sequence.GetDefaultNormalizedEndTime(_Speed) * _Duration;
            _HasEndTime = false;
        ...
        {
            if (EasyAnimatorUtilities.TryGetFrameRate(context.TransitionContext.Transition, out var frameRate))
            {
                seconds = EasyAnimatorUtilities.Round(seconds, 1f / frameRate);
            }
        }
        ...
        private void DoRulerLabelGUI(ref Rect previousArea, float time)
        {
            if (_RulerLabelStyle == null)
                _RulerLabelStyle = new GUIStyle(GUI.skin.label)
                {
                    ...
                    fontSize = Mathf.CeilToInt(EasyAnimatorGUI.LineHeight * 0.6f),
                };

            var text = G2Cache.Convert(time);

            if (_TimeLabelWidthCache == null)
                _TimeLabelWidthCache = EasyAnimatorGUI.CreateWidthCache(_RulerLabelStyle);
            ...
            if (area.x > _Area.x)
            {
                ...
                EditorGUI.DrawRect(new Rect(area.x, tickY, 1, TickHeight), EasyAnimatorGUI.TextColor);
            }
